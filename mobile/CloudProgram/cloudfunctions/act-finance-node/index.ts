...
let myHandler = async function (event: any, context: any, callback: any, logger: any) {
  ...
  try {
    ...
    if (!idempotencyKey) {
      callback({ ret: { code: -1, message: '缺少 idempotencyKey：审批动作必须幂等' } });
      return;
    }
    idemCol = db.collection(IdempotencyRecord);
    const claim = await claimIdempotent(
      idemCol, idempotencyKey, orgId, action, 'approval', instanceId,
      String(JSON.stringify({ instanceId, action, comment })).slice(0, 300), userId,
    );
