  ...
  try {
    ...
    if (action === 'reject') {
      ...
      await storeIdempotent(
        idemCol, orgId, idempotencyKey, 'reject', 'approval', instance.id,
    ...
    await storeIdempotent(
      idemCol, orgId, idempotencyKey, action === 'done' ? 'complete' : 'approve',
      'approval', instance.id,
      { status: finished ? 'approved' : 'approving' }, userId,
    );
