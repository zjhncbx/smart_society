...
let myHandler = async function (event: any, context: any, callback: any, logger: any) {
  ...
  try {
    ...
    obj.version = (obj.version || 1) + (existing ? 1 : 0);
    obj.sourceType = obj.sourceType || 'manual';
    obj.sourceId = obj.sourceId || '';
    obj.code = obj.code || `P-${now.getFullYear()}-${String(now.getTime()).slice(-4)}`;
    obj.updatedAt = now;
    ...
    await recordAudit(
      auditCol, record.orgId, existing ? 'update' : 'create',
      'project', obj.id, obj.name, userId, String(record.actorName || '成员'),
      existing ? existing : null, obj,
    );
