...
function toJson(c: ComplianceItem): any {
  return {
    id: c.id, orgId: c.orgId, code: c.code, name: c.name, itemType: c.itemType,
    deadline: c.deadline ? c.deadline.toISOString() : '', status: c.status,
    responsibleMemberId: c.responsibleMemberId, responsibleName: c.responsibleName, correlationId: c.correlationId,
    createdAt: c.createdAt ? c.createdAt.toISOString() : '', updatedAt: c.updatedAt ? c.updatedAt.toISOString() : '',
  };
}
let myHandler = async function (event: any, context: any, callback: any, logger: any) {
  logger.info('get-compliance-items called');
  try {
    ...
    callback({ ret: { code: 0, message: 'ok', data: { items: rows.map(toJson), total, hasMore: page * pageSize + rows.length < total } } });
  } catch (err: any) {
    logger.error(`get-compliance-items error: ${err.message}`);
