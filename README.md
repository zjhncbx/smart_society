...
│       ├── clouddb/
│       │   ├── db-config.json
│       │   ├── objecttype/         # 13 个对象类型定义（Member/Project/Notice/Org/Finance/BusinessEvent…）
│       │   └── dataentry/          # 种子数据
│       └── cloudfunctions/         # 39 个云函数（含注册登录、财务、审批、结账、事件中心、数据治理等）
└── web/                            # 网页端（规划中）
    └── README.md
```
...
15 个对象类型定义位于 `CloudProgram/clouddb/objecttype/`：
...
39 个云函数，HTTP 触发器、POST、认证类型 `apigw-client`，统一返回 `{ ret: { code, message, data } }`。
...
| 云函数部署 + 真机联调 | ✅ | 39 个云函数 + 15 张表部署至 AGC |
