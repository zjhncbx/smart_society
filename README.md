...
- **自动任务与风险预警（WF-03/04、SA-02/03、SEC-01）**：轻量规则引擎（GR-01~05：任务逾期自动升级、项目进度偏差/延期、审批SLA超时、数据质量自动任务、预算超支），自动生成并升级自动任务与风险/预警（区分风险与预警，去重、自动关闭已恢复项）；自动任务中心（可解释来源规则/SLA/升级路径，完成/取消/重开闭环）+ 风险预警中心（确认监控/标记解决）+ 自动化运行审计日志（每次运行的任务/风险变更与耗时）；首页集成“风险预警”概览卡与自动任务入口
