...
│       │   └── dataentry/          # 种子数据
│       └── cloudfunctions/         # 36 个云函数（含注册登录、财务、审批、结账、事件中心等）
└── web/                            # 网页端（规划中）
    └── README.md
```
...
13 个对象类型定义位于 `CloudProgram/clouddb/objecttype/`：
...
36 个云函数，HTTP 触发器、POST、认证类型 `apigw-client`，统一返回 `{ ret: { code, message, data } }`。
...
| 云函数部署 + 真机联调 | ✅ | 36 个云函数 + 13 张表部署至 AGC |
