...
│       ├── clouddb/
│       │   ├── db-config.json
│       │   ├── objecttype/         # 24 个对象类型定义（Member/Project/Notice/Org/Finance/BusinessEvent/质量/自动化/审计/身份/幂等/工作项等）
│       │   └── dataentry/          # 种子数据
│       └── cloudfunctions/         # 49 个云函数（含注册登录、财务、审批、结账、事件中心、数据治理、自动化治理、审计、身份等）
└── web/                            # 网页端（规划中）
    └── README.md
```
...
24 个对象类型定义位于 `CloudProgram/clouddb/objecttype/`：
...
| 云函数部署 + 真机联调 | ✅ | 52 个云函数 + 24 张表部署至 AGC |
