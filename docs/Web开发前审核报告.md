...
| P0-3 | BusinessEvent 增加 correlationId，打通事件关联链 | §7、§23 | ✅ 已完成（commit：事件关联链路） |
| P0-4 | 冻结云数据契约文档（统一字段清单 + 新增对象规划） | §4、§22 | ✅ 已完成（docs/云数据契约.md） |
| P0-5 | 跨端统一身份（ExternalIdentity 云侧引导层已建；客户端将华为 OpenID 映射为内部 userId 待三端阶段） | 跨端统一用户身份与唯一标识规范 | ⚠️ 云侧基础已完成，客户端迁移待三端开发 |
| P0-6 | 统一工作项 WorkItem 数据模型与查询接口（Web 待办统一消费） | §9 | 大 |
