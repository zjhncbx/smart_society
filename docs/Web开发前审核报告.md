...
3. 本轮统一口径：**云函数 70 个、CloudDB 对象 31 张；全部对象统一字段契约已补齐（用户级 AppUser/UserSettings 豁免、org-keyed 表主键映射）；AuditLog 云侧已闭环；correlationId 全链贯通（待 AGC 部署后真实链路验收）**。
...
| 核验项 | 结论 | 证据 |
|---|---|---|
| ① Cloud DB 对象（31 张表） | ✅ 基础可用 | 31 张表 JSON 可解析；全部对象统一字段契约已补齐（用户级豁免/主键映射） |
